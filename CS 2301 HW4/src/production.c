...
bool production(int argc, char* argv[])
{
	...
	else //there is interesting information
	{
		...
		if((srP->treasure <= maxTreas) && (maxRooms>0))
		{
            ...
            //Put Room 0 in the queue linked list (1)

            Room* roomZero = (Room*) malloc(sizeof(Room));
            roomZero->roomNumber = 0;
            roomZero->treasure = 0.0;
            roomZero->searched = true;

            savePayload(searchQ,roomZero);//TODO: what goes here? //Done?
            srP = (SearchResults*) malloc(sizeof(SearchResults));
            srP->roomNumber=roomZero->roomNumber;
            srP->treasure = roomZero->treasure;
            savePayload2(historyP,srP);//TODO: what goes here? //Done?
		}
			...
			for(int col = 0; (col<nrooms)&&!done; col++)
			{
				...
				if(getEdge(adjMP,roomBeingSearchedP->roomNumber, col)==1)
				{
					...
					{//if it hasn't been searched
						...
						if(foundTreasure + theRoomPs[col]->treasure <= maxTreasure && searchedRooms < maxRooms)//TODO:what's the condition? //Done
							//we check whether we can take the treasure vs. limit - foundTreasure + room.treasure <= maxTreas AND
							//we check whether we've hit the room limit - searchedRooms < maxRooms?
						{//we enqueue it for search
							foundTreasure += theRoomPs[col]->treasure;//TODO what goes here? //Done
                            printf("%f\n", foundTreasure);
							searchedRooms += 1;//TODO what goes here? //Done
							printf("%d\n",searchedRooms);
							printf("found treasure updated to %f.\n", foundTreasure);
							printf("enqueuing room %d.\n", col); fflush(stdout);
							printf("Before enqueuing queue empty reports %d\n", isEmpty(searchQ));
							savePayload(searchQ,theRoomPs[col]);//TODO: what goes here? //Done?
							srP = (SearchResults*) malloc(sizeof(SearchResults));
