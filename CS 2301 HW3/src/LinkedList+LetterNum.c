...
LetterNum* callLetterNum(LLNode* lP){
    LetterNum* mP = (LetterNum*) malloc (sizeof(LetterNum));
    mP->index = 0;
    mP->letter = 90 - (rand() % 25);
    mP->num = (rand() % 9) + 1;
    savePayload(lP,mP);
    return mP;
...
LetterNum* getCurrentCall(LLNode* lP){
    printf("Current Call: ");
    if(lP->payP ==(Payload*)0)
    {
        printf("Empty list\n");
